...
description: Internal Phase 1 of the GTM Brain interview — not a user entry point. Use only after gtm-brain creates or resumes the working doc and the marker says phase: profile-and-goals. Elicits the business layer a non-technical owner drives — business, GTM goals, priority decisions to automate, named tools, and constraints. Hands off to strategy-readout.
---
...
2. **Research the org yourself, following `references/org-research.md`.** That reference is the research procedure and the exact structured shape to produce: from the website URL (and any names given), use whatever web-research/browsing tools this surface provides to build a grounded picture — offering, apparent customers, size signals, visible GTM motions/channels, recent news, a hypothesized lens, and a list of things to confirm. Cite every found claim; keep inferred-vs-found separate; never fabricate. (There is no separate research subagent — you perform this step inline before the interview.)
...
**If web research is unavailable** (no fetch/search/browsing tool exists in this surface): don't block. Tell the owner you couldn't pull their site automatically, ask them to describe the business themselves, and run the interview without the head start.
...
- First, get the real names across the GTM surface: system of record, messaging (email/SMS/push), ads, analytics, data warehouse, enrichment, support, billing (see `../../reference/capability-map.md` for the categories to cover).
...
- Research first, then pressure-test — open from your research findings (per `references/org-research.md`) and challenge what's vague or doesn't add up; never accept a surprising answer at face value. Research is a running start, not truth: the owner confirms everything, and they are the source of truth.
