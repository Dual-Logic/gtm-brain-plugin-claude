---
name: build-spec
description: Internal Phase 3 of the GTM Brain interview — not a user entry point. Use only when the marker says phase: build-spec and the Strategy Readout is confirmed. Drafts the full technical Build Spec on the universal GTM-Brain skeleton, provenance-tags every decision, and refines by having the owner react to proposals rather than authoring anything. Hands off to finalize.
---
...
Load the skeleton (`../../reference/gtm-brain-skeleton.md`), the capability map (`../../reference/capability-map.md`), and the active lens resolved from the marker's `lens:` — a single lens loads `../../reference/lenses/<lens>.md`; a `blend:<a>+<b>` loads **both** `../../reference/lenses/<a>.md` and `../../reference/lenses/<b>.md` and merges their emphasis; `universal` loads no lens file and derives directly from the skeleton (per `../../reference/lens-guide.md`).
...
## Step 1b — Feasibility research (do it yourself, inline)

Before walking the owner through the draft, pressure-test whether it's actually buildable on their stack — don't assume the integrations exist. Perform this research yourself, following `references/feasibility-research.md` (there is no separate feasibility subagent). Taking the draft's capability→tool pairings, the priority decisions, and the unit of decision, research each tool's real integration surface (APIs, webhooks, official MCP servers, reverse-ETL support) and reach a feasibility verdict per capability plus a recommended home for the decision logic. That reference gives the procedure and the exact structured shape to produce.

Fold the findings back into the body:
- **§2.2 capability → tool table** — add each pairing's feasibility note with a citation, tagged `[Proposed]` (the builder verifies exact endpoints), or `[Open - needs your team]` where you found a genuine gap.
- **Open Items** — route every `gap` (a capability with no tool, or a tool with no viable integration for what a decision needs) to `[Open]`, with what's required.
- **Decision-engine home** — use the recommendation to ground how and where the Brain runs (skeleton L7/L8), tagged `[Proposed]`.

**If web research is unavailable** (no fetch/search/browsing tool exists in this surface): don't block — keep the capability→tool mappings as prose-level `[Proposed]`, and add one Open Item noting that integration feasibility (APIs/MCPs) still needs a builder to confirm.
...
- Feasibility is researched, not assumed — your feasibility research (per `references/feasibility-research.md`) enters the spec as `[Proposed]` with citations; genuine integration gaps become `[Open]`, never hand-waved.
