...
Read `../../reference/working-doc-convention.md` for the exact contract (resolve the plugin root from this skill file's installed path; shared references live at `../../reference/`). In short:

- The working doc is a `gtm-brain-spec.md` file at the workspace root when a writable workspace is available (Codex / local); in ChatGPT Work mode it is the uploaded or active spec document, or — with no durable file — the current chat (resume then requires the owner to re-provide the spec). See the convention for the full surface-aware rule.
- **If it does not exist** → fresh start (Step 2).
- **If it exists** → resume (Step 3).

## Step 2 — Fresh start

1. Create the working document from `../../reference/output-template.md` (a `gtm-brain-spec.md` file in a writable workspace, or the active Work-mode document), with the phase-progress marker set to `phase: profile-and-goals`, `last_completed_step: created`, `lens: (undetermined)`.
