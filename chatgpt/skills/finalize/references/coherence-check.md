# Coherence check procedure (Phase 4)

> Read by `finalize`. This is the QA pass the skill performs **itself**, inline, once the full working doc is assembled — it is **not** a separate subagent or subprocess. Run the checks, then apply the fixes yourself.

Review the assembled GTM Brain spec for internal inconsistency and gaps, and produce concrete fixes. This is the last quality gate before the owner walks away with the document — catch the contradictions, drift, and untagged claims that are hard to see after drafting across four phases.

## Purpose

Performed after the full working doc is assembled (Strategy Readout + Build Spec + meta-sections + Open Items). Read the whole document, find where it disagrees with itself or under-delivers on the architecture, and produce a findings list with specific fixes. Do not invent new product content — make the existing spec consistent and honest.

## Inputs

You work from the assembled working doc:

- **working_doc**: the full spec content. This includes the phase-progress marker's `captured_inputs` — **that block is the ground-truth record of what the owner actually said and what research found.** Use it as the source of truth when checking whether specifics in the spec are real.
...
Produce findings in this shape, then apply the fixes (highest-severity first) back into the working doc. This is a working record, not a value returned to a caller:
...
- **Quote the conflicting text** in `issue`/`location` so you can find and fix it precisely.
...
- **Severity honestly.** A readout/build-spec decision mismatch is high; a stray untagged sentence is low. Order matters so the load-bearing ones are fixed first.
